...
export default function TurfMatchReport() {
  ...
  const reportParams = useMemo(() => {
    ...
    if (mode === 'payment') {
      return { from, to, filter_type: 'payment', section: 'turf' };
    }
    return { from, to, section: 'turf' };
  }, [mode, month]);
  ...
  return (
    <div className="page">
      <div className="card-title-row">
        <h2>Turf Match Report</h2>
        <Link to="/report" className="btn small">All Reports</Link>
      </div>

      <div className="card highlight-payment">
        <h3>Month-wise turf matches</h3>
        <p className="hint">
          View turf bookings for a month — by <strong>match date</strong> or by <strong>payments in that month</strong> — then download image / Excel.
        </p>
      ...
      {loaded && (
        <div className="card">
          <div className="card-title-row">
            <h3>
              {mode === 'payment'
                ? `Turf paid in ${formatCoachingMonth(month)}`
                : `Turf matches — ${formatCoachingMonth(month)}`}
            </h3>
          ...
          <SportGroupedTurfTable rows={rows} emptyLabel="No turf records for this month" />
