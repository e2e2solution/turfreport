...
import { FootballCoachingTable } from '../components/ReportTables';
...
export default function FootballCoachingReport() {
  const [mode, setMode] = useState('payment'); // payment | coaching_month
  ...
  const load = async () => {
    ...
    try {
      ...
      setPreviewData(data);
      setRows(data.football_coaching || []);
      setLoaded(true);
    } catch (err) {
      alert(err.message);
    } finally {
  ...
  return (
    <div className="page">
      <div className="card-title-row">
        <h2>Football Coaching Report</h2>
        <Link to="/report" className="btn small">All Reports</Link>
      </div>

      <div className="card highlight-payment">
        <h3>Month-wise list &amp; image</h3>
        <p className="hint">
          View football coaching separately — filter by <strong>payments in a month</strong> or by <strong>coaching month</strong>, then download image.
        </p>
        ...
        <div className="filter-bar" style={{ marginBottom: 12 }}>
          <label>
            ...
            <select value={mode} onChange={(e) => { setMode(e.target.value); setLoaded(false); setRows([]); }}>
              ...
              <option value="coaching_month">Coaching month list</option>
            </select>
          </label>
          <label>
            Month
      ...
      {loaded && (
        <div className="card">
          <div className="card-title-row">
            <h3>
              {mode === 'payment'
                ? `Paid in ${formatCoachingMonth(month)}`
                : `Coaching month ${formatCoachingMonth(month)}`}
            </h3>
          ...
          <div className="stat-grid" style={{ marginBottom: 12 }}>
            <div className="stat-card">
              <span className="stat-label">Package Total</span>
          ...
          <FootballCoachingTable rows={rows} />
